...
{
	draw_text(x + tspace, y + (m * tspace), string(title[m]));
}
draw_set_font(fnt_credits);
for (n = 0; n < array_length_1d(pers); n += 1)
{
	draw_text_ext(x + tspace + nspace, y + (n * tspace) + nspace, string(pers[n]), nspace, 1000);
}